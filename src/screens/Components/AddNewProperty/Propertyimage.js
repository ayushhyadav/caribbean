...
export default class Propertyimages extends Component{
    ...
    render(){
        return(
          ...
          <label style={{marginLeft:20}}>Step 3/4</label>
       ...
       <button style={{width:'10%',borderRadius:10,height:'45px',borderWidth:0,background:'white' }}
        onClick={()=>this.props.NextCallBack({navigationTo:'property_Images',id:2,currentid:3,bt_type:'Back'})}
       >Back</button>
                    <button style={{color:'white',background:'#F15A29',width:'10%',borderRadius:10,height:'45px',marginLeft:10 }}
                     onClick={()=>this.props.NextCallBack({navigationTo:'Extra_Service',id:4,bt_type:'Next'})}
                    >Next</button>
                    </div>
         </div> 
        )
