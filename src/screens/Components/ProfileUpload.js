...
export default class ProfileUpload extends Component{
    ...
    render(){
       ...
       onClick={()=>this.props.NextCallBack({navigationTo:'Per_info',id:1,currentid:2,bt_type:'Back'})}
       >Back</button>
                    <button style={{color:'white',background:'#F15A29',width:'10%',borderRadius:10,height:'45px',marginLeft:10 }}
                    onClick={()=>this.props.NextCallBack({navigationTo:'Veri_Phone',id:3,bt_type:'Next'})}
                    >Next</button>
                    </div>
         </div> 
